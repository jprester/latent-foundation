...
1. **Ideator** — invoke the `scp-ideator` subagent. Pass the raw user input (may be empty, a vague theme, or theme + class). It returns a *premise spec*: title, class, refined theme, 1–2 setting details, tag suggestions, and a short rationale for why this premise will work as an SCP entry.

2. **Writer** — invoke the `scp-writer` subagent with the premise spec. It returns the full story body in markdown (no frontmatter, no surrounding commentary).

3. **Save** — write the file to `stories/scpg-NNN.md` with correct frontmatter (see below). NNN is the next available number.

4. **Thumbnail** — shell out to the existing script:
   ```bash
...
Notes:
- `title` is **always** `"SCPG-NNN"` — the in-story title comes from the body's `# SCP-XXXX: "..."` heading. Do not put the prose title in the `title` frontmatter field.
